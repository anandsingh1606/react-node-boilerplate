...
export default {
  ...
  mobileNumber:(filed = 'mobileNumber') => {
    ...
    .isMobilePhone().withMessage('Mobile number is not valid')
  },
  countryCode:(filed = 'countryCode') => {
    return check(filed).not().isEmpty().withMessage('Country code is required').isInt().withMessage("Country code is not valid");
  },
  displayName:(filed = 'displayName') => {
    return check(filed).not().isEmpty().withMessage('Display Name is required')
  },
  otp : (filed = 'otp') => {
    return check(filed).not().isEmpty().withMessage('Otp is required').isInt(6).withMessage("Otp is not valid");
  },
};
